...
// ====== SOLVE FUNCTION ======
void update (int p, int q, int x, int y, int& best_cost, int& bestP, int& bestQ)
{
    if(!(p&q))
    {
...
void david_vivek()
{
    ...
    int best_cost = 1e12;
    int bestP = 0, bestQ = 0;
    update(x,y,x,y,best_cost,bestP,bestQ);

    brep(i,30,0)
    {
        if( ((x&y)>>i) & 1 ) 
        {
            update( (x>>i<<i)+(1<<i) , y , x,y,best_cost,bestP,bestQ);
            update(  x, (y>>i<<i)+(1<<i) , x,y,best_cost,bestP,bestQ);
            update(  x>>i<<i, (y>>i<<i)-1 , x,y,best_cost,bestP,bestQ);
            update(  (x>>i<<i)-1, y>>i<<i , x,y,best_cost,bestP,bestQ);
        }
    }
    cout << bestP << " " << bestQ; nl;
}
