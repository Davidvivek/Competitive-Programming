...
void david_vivek()
{
    ...
    int a0=0, b0=0;
    for(int i=0; i<n; i+=2) if(a[i] == '0')a0++;
    for(int i=0; i<n; i+=2) if(b[i] == '0')b0++;

    int a1=0, b1=0;
    for(int i=1; i<n; i+=2) if(a[i] == '1')a1++;
    for(int i=1; i<n; i+=2) if(b[i] == '1')b1++;
    ...
    int ans=0;
    {
        ...
        {
            while (p<x.length() && x[p] != '1') p++;
            while (q<y.length() && y[q] != '1') q++;
