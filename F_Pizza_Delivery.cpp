...
void david_vivek()
{
    ...
    for(auto it:mx)
    {
        ...
        int needed = (cx-lx) + (hi-lo);
        cnt++;
        
        dp[0][cnt] = min(dp[0][cnt-1]+abs(hi-mn[lx]),dp[1][cnt-1]+abs(hi-mx[lx]))+needed;
        dp[1][cnt] = min(dp[0][cnt-1]+abs(lo-mn[lx]),dp[1][cnt-1]+abs(lo-mx[lx]))+needed;
