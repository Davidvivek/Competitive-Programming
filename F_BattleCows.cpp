...
void david_vivek()
{
    ...
    rep(i,0,q)
    {
        ...
        {
            int block_idx = p>>k;
            int sibling_idx = block_idx^1;

            int myL = block_idx * (1<<k);
            int myR = myL + (1<<k) - 1;
            int myVal = (pref[myR+1]^pref[myL]) ^ delta;

            int sibL = sibling_idx * (1<<k);
            int sibR = sibL + (1<<k) - 1;
            int sibVal = (pref[sibR+1]^pref[sibL]);

            bool win = false;
            if(block_idx < sibling_idx && myVal>=sibVal) win = true;
            if(block_idx >= sibling_idx && myVal>sibVal) win = true;
