...
void david_vivek()
{
    ...
    for(int i=1; i<=n; i++)
    {
        int len = r[i] - l[i] + 1;
        int val = len*len - len;
        a.push_back({{l[i], r[i]}, val});
        
    }
    sort(a.begin(), a.end(),[](auto &x, auto &y){return x.first.second < y.first.second;});
    ...
    for (int i = 1; i <= n; i++)
    {
        int left = a[i-1].first.first;
        int val = a[i-1].second;

        int pos = lower_bound(ends.begin(), ends.begin()+i-1, left) - ends.begin();

        dp[i] = max(dp[i-1], dp[pos] + val);
    }
