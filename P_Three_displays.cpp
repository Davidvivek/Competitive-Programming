...
void david_vivek()
{
    ...
    rep(j,1,n-1)
    {
        int leftmin = LLONG_MAX;
        int rightmin = LLONG_MAX;

        rep(i,0,j)
        {
            if(s[i]<s[j])
            {
                leftmin = min(leftmin , c[i]);
            }
        }
        ...
        rep(k,j+1,n)
        {
            if(s[j]<s[k])
            {
                rightmin = min(rightmin , c[k]);
            }
        }

        if(leftmin != LLONG_MAX && rightmin != LLONG_MAX)
        {
            ans = min(ans , leftmin + rightmin + c[j]);
        }
    }
