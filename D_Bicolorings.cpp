...
// ====== SOLVE FUNCTION ======
int getBit(int t, int pos) 
{
    if (t == 0) return 0;           
    if (t == 1) return 1;          
    if (t == 2) return (pos == 0 ? 0 : 1); 
    return (pos == 0 ? 1 : 0);     
}
int getdelta(int prev, int curr) 
{
    int pt = getBit(prev, 0);
    int pb = getBit(prev, 1);
    int ct = getBit(curr, 0);
    int cb = getBit(curr, 1);
...
void david_vivek()
{
    ...
    vector<vvi> dp(n+1,vvi (k+2, vi(4,0)));
    dp[1][1][0] = 1;
    dp[1][1][1] = 1;
    dp[1][2][2] = 1;
    dp[1][2][3] = 1;
    ...
    for(int i=2; i<=n; i++)
    {
        for(int j=1; j<=k; j++)
        {
            for(int prev = 0; prev<4; prev++)
            {
                ...
                {
                    int delta = getdelta(prev , curr);
                    if(j-delta >= 0)
                    {
                        dp[i][j][curr] = (dp[i][j][curr] + dp[i-1][j-delta][prev])%mod;
                    }
                }
            }
        }
